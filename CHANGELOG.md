# Changelog

## v1.7.5-beta.6（2026-09-21，测试预发布）

> 两项修复：流式 tool_calls 增量丢工具名（grok CLI 工具调用全失效）+ 插件启停状态跨进程竞态。

### 🐛 修复

- **流式 tool_calls 增量丢工具名（逐帧取值客户端工具调用全失效）**：OpenAI 流式约定同一
  工具调用只有首帧带 `id` / `type` / `function.name`，后续帧只有 `arguments` 增量，客户端
  应自行累加。但部分客户端（实测 grok CLI 1.0.34）逐帧取值、被空字符串覆盖，最终工具名
  为空报 `Tool not found`。现网关流式转发按请求持有状态、按 `index` 记住首帧头字段并在
  后续帧补齐（`normalizeToolCallDeltas`）；对正确累加的客户端无副作用。responses / claude
  两条流路径本就跨帧聚合，不受影响
- **插件启停状态被陈旧快照写回旧值（跨进程竞态）**：经管理 API 启用某插件后，状态文件可能在
  十几秒内被其它实例写回 `false`、插件被停（2026-09-21 现场：vibex；同一轮连续 toggle 6 次复现 1 次）。
  根因是跟随路径（`applyStateChanges`）用函数开头的快照值落盘，而 `Toggle` 落盘前会先 `killCurrent`
  （`taskkill` 可能耗时数秒），窗口内用户经另一进程改的新值被旧快照覆盖。现改为：跟随路径
  **预检 + 写时 CAS**（`updateStateFileCAS`），磁盘值 ≠ 期望值则放弃落盘；用户 toggle 路径不变
  （无条件生效）。写盘/跳过补 `id/enabled/pid` 日志。详见 `docs/PLUGIN-PROVIDERS.md` §4.4

## v1.7.5-beta.5（2026-09-19，测试预发布）

> 修复打包版（Tauri 壳）内插件面板入口点击无反应的问题；随版附插件功能建议清单（§11）
> 与通用契约冒烟测试脚本。

### 🐛 修复

- **打包版插件面板入口点击无反应（Tauri WebView 拦截 window.open）**：管理 UI 跑在
  Tauri WebView 内时，`window.open` 打开的跨源新窗口被静默拦截（dev 下前端经浏览器
  访问故仅打包版暴露）。现由 core 新增 `POST /api/admin/open-url`（登录鉴权域内）代为
  调用系统默认浏览器；仅允许 http/https 的 127.0.0.1/localhost/[::1] 回环地址，拒绝
  引号/尖括号/换行（防 cmd 参数逃逸）。前端打开插件面板统一走该端点，失败时兜底
  `window.open`（浏览器直访模式）

### 📝 文档与工具

- **docs/PLUGIN-PROVIDERS.md §11 插件功能建议清单**：提炼自 mmxproxy / skywork 两个
  参考实现——P1 强烈建议（内嵌面板三卡 / 面板入口声明 / provider.json 自管理 / 账号池 /
  网络管理 / stderr 纪律 / 契约冒烟测试）、P2 按需（对话调试 / 凭证自动恢复 / 号池守护 /
  用量统计 / 网络设置分离持久化 / 专属分组建议 / 上下文信息透出）、源码组织两条已验证
  路线（fork+同步器 vs 同仓库独立模块；单源码双模式已回退）
- **scripts/plugin-smoke-test.sh 通用契约冒烟测试**：模拟宿主拉起插件，验证就绪行
  （port/auth 回显/id 一致）、鉴权 401、端点可达、面板可达；已对 skywork 插件实测
  通过 7/0/0

## v1.7.5-beta.4（2026-09-19，测试预发布）

> 插件生态两件套：管理面板入口 + 模型元数据；外加一次误路由事故的根修与
> 插件功能建议清单（docs/PLUGIN-PROVIDERS.md §11）。

### ✨ 新增

- **插件卡片管理面板入口（球形图标）**：插件可用 `panel:"sidecar"`（面板挂插件子进程
  动态端口）或 `panel_port:N`（固定端口，0=禁用）声明自带控制面板；卡片渲染 🌐 图标，
  点击新窗口打开并把网关地址 / 实例密钥（打码）/ 暴露模型清单经查询参数带给面板；
  sidecar 模式点击时实时解析最新端点，规避子进程重启换端口导致的死链
- **插件模型上下文信息**：插件 `/v1/models` 可携带 `context_window` / `max_output_tokens`，
  经聚合目录透传——管理页插件卡片显示最大上下文、暴露弹层逐模型显示、对外 `/v1/models`
  输出 OpenRouter 风格 `context_length` 扩展字段（OpenAI 兼容客户端自动忽略未知字段）
- **插件功能建议清单（§11）与通用契约冒烟测试**：docs/PLUGIN-PROVIDERS.md 新增 §11
  （提炼自 mmxproxy / skywork 两个参考实现的 P1/P2 功能点与源码组织建议）；
  `scripts/plugin-smoke-test.sh` 模拟宿主拉起验证插件契约（就绪行 / 鉴权 / 端点 / 面板可达）

### 🐛 修复

- **带厂商前缀的模型不再误兜底默认厂商（误路由事故根修）**：插件停用/缺席期间，
  `skywork/*` 类请求此前会兜底落到 opencode 上游——产生误导性 401（Model not supported）
  并把 opencode 账号池误打入坏池。现改为：前缀命中已注册厂商 → 直连该厂商（目录本轮
  拉取失败 ≠ 厂商不可用）；前缀厂商未注册 → 返回明确 503；无前缀模型保持原兜底行为
- **插件模型清单拉取失败不再永久为空**：管理器就绪后拉取插件模型目录改为退避重试
  （0/5s/15s/30s + WARN 日志）——此前单次超时会导致面板「暴露模型」按钮永久禁用

### 📝 文档

- README 构建章节补充 core 重嵌入警告：重编 core 后需 touch Rust 源文件或 cargo clean，
  否则壳内仍是旧 core
## v1.7.5-beta.3（2026-09-18，测试预发布）

> 上游 opencode 免费通道再次加码：新增请求体级门禁（强制流式 + 工具名指纹），本版适配后免费模型恢复可用。

### 🐛 修复

- **opencode 免费模型再次 403 FreeTierError（请求体级门禁）**：上游自 2026-09-18 起在身份头之外新增两项
  请求体校验——① 上游请求体必须 `stream:true`（非流式即使其余全对也一律 403）；② `tools` 必须包含
  `bash` / `glob` / `grep` / `read` 四个名字（大小写敏感，schema 与描述不限）。本版对匿名免费通道
  自动补齐这四个占位工具（客户端已有同名工具保留不覆盖）、强制上游流式，并在网关内把上游 SSE 聚合回
  非流式 JSON（`chat.completion`，保留 content / reasoning / 工具调用 / usage）；客户端未提供工具的
  纯聊天额外下发 `tool_choice:"none"`，避免模型调用占位工具。chat 与 responses 两条上游路径同步生效，
  付费通道（zen/go 带 key）请求形态不变。真机复验：非流式下游与流式下游均 HTTP 200

## v1.7.5-beta.2（2026-09-17，测试预发布）

> 上游 opencode 免费通道新增服务端身份门禁，导致全部免费模型 403；本版适配后恢复可用。

### 🐛 修复

- **opencode 免费模型恢复可用（403 FreeTierError）**：上游自 2026-09-16/17 起对匿名免费通道
  （`Authorization: Bearer public`）增加两项服务端校验——① `x-opencode-session` 必须为官方身份格式
  `ses_` + 12 位十六进制 + 14 位 base62（共 30 字符）；② `User-Agent` 必须为 `opencode/<版本>` 且
  版本 ≥ 1.17.0（低于该版本返回 426 UpgradeRequired，无版本号或第三方 UA 返回 403 FreeTierError）。
  本版按官方规范生成会话与请求 ID（`x-opencode-request` 同为 `msg_` + 26 位身份串），并把版本探测
  兜底值提升到门禁下限之上（探测到偏低版本时按 1.17.0 上报）；chat 与 responses 两条上游路径同步生效。
  真机复验：`mimo-v2.5-free` 恢复 HTTP 200（修复前同场景 curl 实测 403）

## v1.7.5-beta.1（2026-09-15，beta）

> 插件式供应商体验增强：模型清单手动刷新。

### 🚀 新功能

- **插件式供应商「刷新模型」按钮**：「暴露模型」弹层头部新增刷新入口，点击后经插件子进程
  的标准 `/v1/models` 接口实时重拉上游（官网）最新模型清单，同步更新弹层列表、卡片模型数与
  聚合目录（网关 `/v1/models` 即时生效）；刷新失败保留旧清单仅提示错误，已勾选白名单自动与
  新清单取交集。新后端 API：`POST /api/admin/plugins/{id}/refresh-models`（鉴权同管理面）。
  插件侧零改动、零特化——对所有 OpenAI 兼容插件通用。

## v1.7.4（2026-09-09，正式发布）

> 由 v1.7.4-beta.1/2/3 验证后合入 main 的正式版：可观测性机制 + opencode 免费模型（muse-spark）完整可用。

### 🚀 新功能

- **opencode muse-spark 系列免费模型可用**：上游仅通过 OpenAI Responses 端点（`/zen/v1/responses`）提供
  该模型，此前网关固定走 chat/completions 恒 500。现按模型自动路由 responses 端点（付费 go 面走
  `/zen/go/v1/responses`），并在厂商内完成 chat ↔ responses 双向翻译（非流式 JSON + 流式 SSE），
  三种下游协议（OpenAI chat / Anthropic / Responses）统一可用
- **工具调用 / agent loop 闭环**：客户端 tools/tool_choice 透传到上游，历史 assistant tool_calls 与
  tool 结果映射为 Responses items；返回侧还原 `message.tool_calls`（非流）与流式 `function_call` 事件，
  模型从"只口头应答"变为真正可调用工具并基于结果续答
- **usage 明细透出**：muse 的推理 token 数以 `completion_tokens_details.reasoning_tokens` 透传，
  客户端可见"思考了多少"
- **可观测性机制**（beta 系列并入）：调用记录归因字段/路由结论/统一日志/分布式 Trace/诊断/失败现场打包/
  配置溯源/失败计数/复现重放/预内容静默超时

### 🐛 修复

- **muse-spark-1.3-contributor-free 恒 500**：chat/completions 端点上游不支持该模型（任何出口均 500/
  403），改走 responses 端点后正常
- **assistant 历史消息 400**：Responses 协议按角色限制 content part 类型，assistant 历史必须用
  `output_text`（误用 `input_text` 上游 400）
- **tool_choice 400**：muse 上游仅支持 `tool_choice:"auto"`，`none/required/指定函数` 归一化处理
  （none → 本轮不下发工具；required/指定函数 → 回退 auto 决策）
- **首字慢归因**：muse 为长思考模型（默认 effort=high，单次数百 reasoning token 后吐正文），
  思考段上游仅下发加密内容，属上游特性；外部 `reasoning_effort=low/medium/high` 已确认透传生效

### 🔧 内部/规范

- `vendors/opencode` 新增 responses 端点支持层（模型判定/端点选择/请求构造/JSON 与 SSE 翻译）
- Go 测试全绿；真机 e2e 验证：非流/流式/Anthropic 三协议 + 带工具 agent loop 闭环

## v1.7.4-beta.3（2026-09-05，测试预发布）

> 合入 `feat/debug-observability` 调试与可观测性机制（阶段1-8）。验证通过后再合入 main 打正式 `v1.7.4`。

### 🚀 新功能

- **调用记录可观测字段**：`CallRecord`/`CallLogRecord` 补 `tier`/`serving_port`/`route_verdict`；版本打印注入 branch/commit/built
- **路由结论判定**：`route_verdict` 字符串枚举（proxied / direct_by_design / direct_config_missing / direct_unexpected），在空节点兜底成「直连」前判定，日志可区分「设计直连」vs「配置丢失直连」；前端四类标签 + 按层分组统计
- **统一归因日志（阶段2）**：动态日志级别热切换（`-debug-subsystem` + `POST /api/admin/debug`）+ role/node/tier/provider/port/trace_id 归因字段
- **分布式 Trace（阶段3）**：`X-Trace-ID` 跨进程传播，网关/实例/探针子进程透传 `OPENCODE2API_TRACE`
- **诊断端点 + doctor（阶段4）**：`/api/diag` + `opencode2api doctor` 体检报告
- **失败现场打包（阶段5）**：关键失败自动落脱敏 postmortem bundle（trace_id/路由决策/配置快照），异步不阻塞主请求
- **配置溯源（阶段6）**：配置写入带 writer/时间戳/脱敏快照；`/api/config/history` + `/api/config/effective`（一致性 + 键级 diff）
- **失败原因计数器（阶段7）**：节点 × 原因（429/401/503/connect/timeout）二维计数 + `/api/admin/fail-stats` + 统计页卡片
- **复现重放（阶段8）**：`debug_replay` 子命令——从现场包生成脱敏 fixture 并重放（带同一 X-Trace-ID 便于日志对照）
- **预内容静默超时**：子代理/工具调用长思考场景（已连接未吐正文）默认 30s，避免误杀成 0 token 空回复

### 🔧 内部/规范

- `architecture.md` / `coding-standards.md` / `.opencode/agents` 修正错误技术栈污染（Go 1.22 纯标准库 + React 19 + Tauri 2 + Tailwind 4）
- `npm run build` / `go test` 通过；真机端到端（真实上游 503 / Rust 壳写者接线 / 端到端重放）待验证

## v1.7.4-beta.2（2026-09-02，测试预发布）

> 测试分支 `test/key-health-resume-fix`。验证通过后再合入 main 打正式 `v1.7.4`。

### 🚀 新功能

- **自定义源 401/403 失效恢复机制**：`key_403_cooldown`（秒）可配置——>0 时失效 key 冷却到期自动回池重试（默认 600s），不再「一次 403 永久禁用」；=0 保持旧行为。多 key 时仅失效 key 被标记，其余正常服务
- **手动「恢复全部 Key」**：列表卡片新增按钮，一键清除该源全部 key 运行态（禁用/限流冷却/403 冷却/熔断），无需等待冷却或编辑保存

### 🐛 修复

- **自定义源 403 后全 502**：单 key 源首次 403 被永久禁用 → 后续请求 `withKeys` 无可用 key 返回 `(nil,nil)` → 网关合成裸 502。改为冷却恢复后，冷却中请求透传明确错误「所有 key 均不可用（401/403 冷却或禁用中）」，不再裸 502 无原因
- **key 健康计数分类**：`KeyPoolStatus` 拆分 `auth_cooling`（401/403 冷却）与 `circuit_open`（连续失败熔断），不再与 429 限流冷却混计；UI 列表区分显示「限流冷却 / 403冷却 / 熔断 / 禁用」
- **health 探测 403 误伤**：后台探测把 403 与 404 同等对待（该 key 不提供探测模型/无权限），不再把订阅差异 key 误判为坏 key

## v1.7.4-beta.1（2026-09-02，测试预发布）

> 测试分支 `test/key-health-resume-fix`。验证通过后再合入 main 打正式 `v1.7.4`。

### 🚀 新功能

- **自定义源 key 健康优先策略**：`key_strategy: health`，后台探测 + 流失败计入健康；保存后热生效
- **key 连续失败熔断**：连续 3 次失败冷却 30s，`round_robin` / `failover` / `health` 各策略通用

### 🐛 修复

- **流续写双回复**：会话级 key 粘性，续写强制复用原 key，不再换 key 导致重复输出
- **health 策略校验与回显**：保存放行 `round_robin|failover|health`；面板不再把 health 强转成其他策略
- **usage 内 null 字段**：清洗后兼容严格客户端（Grok serde `u32` 解析）

## v1.7.3（2026-08-31）

### 🚀 新功能

- **测试拉取模型支持走本机系统代理**：自定义供应商「测试并获取模型」新增「走本机系统代理」开关（`use_local_proxy`，走 `http.ProxyFromEnvironment`），与「走节点池 via_proxy」区分；仅测试动作生效、不写入配置

### 🐛 修复

- **重启后 OpenCode 模型全部丢失（只剩 windsurf/自定义/插件模型）**：OpenCode 目录磁盘缓存（stale-while-revalidate）——重启后聚合器首次刷新即带上上一代 OpenCode 模型；`fetchOCVersion` 版本探测独立 5s 短超时 + 非 2xx 回退默认版本，不再被慢上游拖住；启动期目录刷新异步化（先监听后刷新），上游不可达不阻塞进程监听
- **多 key 自定义供应商「串」对话（key1 答一半、key2 不接续、输出重复）**：会话级 key 粘性 + 流式断点续写强制复用原 key（`PreferredKeyIdx` 经 Extra 透传），同一请求续写不再轮询换 key；调用日志增加 key 末位标识便于定位
- **多实例节点扎堆（10 实例只用 2-3 个）**：smart 路由每请求推进游标轮询健康节点（保留健康/坏池/熔断/超时切换，单节点退化恒命中该节点）；质量加权同档加随机抖动分散，显著分差仍优先高分

### 📌 说明

- 从 `feat/fix-startup-catalog-block` 分支摘取启动修复核心 4 类改动（磁盘缓存/版本超时/异步化/回写收窄），不整支合并
- 全程经独立测试（big-pickle test-engineer）/代码审查（big-pickle code-stylespector）验收通过，`go test -count=1 ./...` 13 包全绿、`-race` 干净、前端 `npm run build` 通过
- 记录于 `docs/issue-log/2026-08-31.md`

## v1.7.2（2026-08-29）

### 🐛 修复

- **实例池「15s 内未能监听」100% 启动失败（根因修复）**：自定义供应商上游直连不可达时，实例子进程冷启动会同步拉取模型目录（最长 60s）迟迟不监听端口，manager 15s 就绪等待必然超时。启动期目录刷新异步化（先监听后刷新），`/v1/models` 由既有冷启动防惊群路径 + 磁盘缓存兜底；停止/删除实例后重建不再全挂
- **插件状态文件多进程并发丢开关**：`.plugin-state.json` 写入改「写前重读合并 + 唯一临时文件 + rename 原子替换 + 有界重试」，读合并写段持锁串行化——主管理器与实例/网关子进程的开关不再互相覆写（撕裂文件曾致已停用插件被批量误拉起）
- **runtime 配置统一原子写**：实例/网关/自定义源补丁/auto 补丁四处裸写改原子写；子进程启动回写收窄为「文件健康时跳过」，杜绝启动竞态丢自定义源
- **插件孤儿周期回收**：实例被强杀后遗留的插件副本由扫描循环周期回收，不再无限期存活
- **自定义源「测试并获取模型」死循环**：取消「全部暴露」后可直接点测试拉清单（原前端校验误拦测试路径，保存路径拦截保持）
- **编辑已有白名单的源切「全部暴露」不生效**：保存时显式清空旧白名单（原「键缺失=保留」语义致选择被静默忽略）

### 📌 说明

- 修复分支 `feat/fix-startup-catalog-block`，全程经独立测试/审查/验收流水线，记录见 `docs/issue-log/2026-08-28.md`、`docs/issue-log/2026-08-29.md`

## v1.7.1（2026-08-28）

### 🚀 新功能

- **auto 模型白名单化**：自定义虚拟模型名、勾选参与、无候选明确报错
- **自定义供应商模型列表搜索框**：400+ 模型场景即时过滤，大小写不敏感

### 🐛 修复

- **多 key 模型目录取并集 + 按模型亲和路由 + 404 换 key 兜底**
- **聚合器按厂商保留 lastGood 目录**：消除新增自定义供应商后网关旧模型 502
- **自定义供应商保存/传播/重建/目录刷新补诊断日志**（502 排障）


## v1.6.0（2026-08-20）

### 🚀 新功能

- **插件式供应商（R 系列）**：把"供应商"做成即插即用插件——`providers/<id>/` 放一个侧车可执行文件 + `provider.json`，主进程自动发现、拉起、注册进聚合器（模型目录 / failover / 统计 / 日志全链路复用）
  - 侧车契约：`--provider-serve --port 0` 启动、stdout 就绪行（ready/need_config/fatal）、HTTP `GET /v1/models` + `POST /v1/chat/completions`（SSE 流）；支持任意语言实现
  - 「自定义模型」页双标签 UI：插件式供应商 tab（状态徽标/筛选/JSON 编辑/启停/删除二次确认）+ 原自定义供应商 tab
  - 插件模型可从主管理器、实例子进程、统一网关任一入口使用；启停开关跨进程全局一致生效（≤3s 跟随），关闭后所有入口即时不可用
  - 生命周期完善：反复启停杀净子进程（无端口残留）、启动自动回收强杀遗留孤儿、插件停用状态持久化（重启保持）

### 🔧 修复

- **链路质量探测黑盒 0 分（问题 1）**：探测改经实例 API 端口（带实例 key，标准 net/http），把「实例进程+鉴权+出口+上游」整条链纳入；失败原因透传到质量徽章悬停展示，不再是无从排查的 0 分
- **实例测试误报「无免费模型可测试」（问题 5）**：`httpRequest` 支持 `Transfer-Encoding: chunked` 响应解码（Go 服务端大响应走 chunked，裸 TCP 读入 chunk 元数据导致解析失败）
- **实例池「网关可用免费模型」/ auto 列表缺失自定义与插件模型（问题 6）**：去掉 `fetchGatewayModels` 对上游已过滤免费目录的二次 `isFreeModelID` 误杀
- **流式对话中途节点故障不切节点（问题 7）**：识别上游流中途 SSE `error` 事件（额度用尽/限流/5xx）→ 视为中断走续写 + 切节点；请求级错误（400 类）保持原样透传
- **自定义模型源「测试并获取模型」放开源 ID 校验（问题 3）**：测试仅需 base_url + key（后端 `_test` 占位），保存时仍强制 ID 合规
- **日志页统计口径互斥化（问题 2）**：失败/异常切换/成功三类不再重复计数

### 📌 说明

- 正式版安装包与便携包均不含任何第三方供应商插件（插件由用户自行放入 `providers/` 目录）

## v1.5.4（2026-08-19）

### 🚀 新功能

- **统一网关端口自定义**：实例池页「统一网关」卡片新增「自定义端口」入口，保存即热生效（`ApplyPort` 热重启，镜像密钥行为）；端口配置走 `env > config > 默认` 三源优先级，重启后不再被 env 固化旧值
- **deb 包自动注册 systemd 服务**：`.deb` 安装后自动注册并启用 `opencode2api` 服务（`systemctl status opencode2api` 查看），默认端口 60000、密钥 `sk-unified-local`，支持 `OPCODE2API_GATEWAY_KEY` env 注入；修改 `/etc/opencode2api/manager.env` 后需 `systemctl daemon-reload && systemctl restart` 才生效
- **Linux/Headless 增强**：`--headless` 无头模式支持（释放内嵌组件 → 拉起 core → 打印管理地址 → 退出自动清理）；Linux 平台二进制解析与可写目录回退（deb/NSIS 安装场景自动复制前端 dist）
- **管理鉴权默认关闭**：裸 core 不再要求 `-password`（默认空 = 不启用登录验证），需要时显式 `-password <密码>`
- **deb 安装完成输出醒目提示**：`postinst` 打印经理.env 路径、重启命令、状态/日志查询命令与管理 WebUI 地址

### 🔧 修复

- **实例池测试误报「无免费模型可测试」**：免费模型名单扩充 `hy3`、`nemotron-3.5-lightning`（上游新增）；`pickFreeModel` 加兜底——本系统 `/v1/models` 已只返回免费模型，data 非空时跳过 auto 虚拟模型取首个可测模型，硬编码名单再过期也不误报
- **网关自定义密码重启失效**：双 config 结构（`AppConfig` / `core/manager.Config`）共用 `config.json` 相互覆盖，改读-合并-写（`MergeConfigJSON`）保留对方字段
- **上游失败响应体透传入调用日志**：日志错误信息完整显示 `upstream status <码>: <原因> body=<上游响应体>`，不再只有 `upstream status 400: <nil>`
- **deb 维护脚本 CRLF 修复**：`.gitattributes` 强制 sh/service/env 文件 LF，修复 dpkg 无法执行 `preinst`（shebang 带 `\r`）的问题

### 🎨 UI/UX

- **七页标题/图标/布局统一**：统一 h1 字号与 teal 图标、去除页面宽度限制（全宽一致）
- **七页按钮尺寸统一至独享页基准**：工具条 `px-3 py-1.5 text-[13px]` / 行内 `px-2.5 py-1 text-[12px]` / 弹窗 `px-4 py-2 text-[13px]`
- **弹窗遮罩防误关**：含表单弹窗点击遮罩不再关闭（含统一网关端口弹窗），防填写中误触丢失输入
- **设置页精简**：移除代理出口区块与退出程序按钮，新增版本标识

### 📦 打包与文档

- CI 增加 `workflow_dispatch` 手动触发；`gitignore` 忽略 linux-out 构建产物与测试图片
- README 同步：deb systemd 服务说明、`apt install ./xxx.deb`、`manager.env` 修改后重启提示、config.json 统一网关端口/密钥说明

### 🙏 致谢

- 感谢 **FYHC1** 贡献 Linux/Headless 支持、deb systemd 部署、统一网关端口/密钥持久化与本版全部 PR（#12–#15），已完成代码审查并合入
- 感谢 **Sujinxin123** / **6Kmfi6HP** 的开源基础与历史贡献（统一网关、免费额度实测健康检查、Go 代理核心）
- 感谢所有反馈与测试的使用者

### 🚀 新功能

- **订阅分组命名增强**：分组名优先取订阅内容里的配置名（`# Profile:` 注释 / 顶层 `profile.name`），其次才按文件名 / URL 末段推导；新增订阅对话框支持手动指定分组名（固定后自动拉取不覆盖）

### 🔧 修复

- **节点扫描 sing-box 启动即崩**：hysteria2 带宽字段（up/down）改为数值 Mbps——字符串会被 sing-box 直接拒绝；vless reality 的 `utls` 移入 `tls` 子对象——原先放 outbound 顶层导致 unknown field 报错
- **探测提示文档化**：FAQ 新增「models 接口连通（无免费模型可测试）」的含义说明与改进方向
- **Linux `--headless -port` 未生效修复**：壳的健康检查/地址打印/窗口导航未对齐透传的 `-port`（core 实际监听指定端口，壳仍探测默认 40000）——现由 `override_port` 解析透传端口并贯穿三处调用点，Debian/Ubuntu 等 Linux 无头模式按实际端口就绪、打印与导航

### 🙏 致谢

- 感谢 **FYHC1** 贡献订阅分组命名与扫描 probe 修复（PR #10 / #11），已完成代码审查并合入

## v1.5.2（2026-08-18）

### 🔧 修复

- **调用日志「清空日志」修复**：运行中的实例/统一网关进程不再直写其日志文件，一律走 HTTP 清空——修复 Windows 下「文件被占用」导致清空失败

### 🧹 仓库整理

- 研发过程文档（MASTER-PLAN / REQ-\* / REVIEW-\* 等）移出仓库：本地保留、gitignore 忽略；仓库内文档聚焦产品契约（API / CONFIGURATION / ROUTING / CUSTOM-MODELS / DEPLOYMENT / FAQ 等）

## v1.5.1（2026-08-17）

### 🔧 重要修复

- **节点扫描（探测）修复**：探测子进程配置不再注入厂商 providers——修复此前所有节点扫描判定失败（可用 0 个）；节点可用性判定改为 `/v1/models` 返回 2xx 即可用（能连通上游就算可用，免费模型 chat 测试仅作额外验证）
- **统计页「重置统计」修复**：运行中实例/统一网关一律走 HTTP 复位，不再直写其统计文件——修复 Windows 下「文件被占用」导致无法重置；统一网关端口按环境槽位生效（env > config > 默认），dev/便携/web-dev 不再打错端口回退直写；实例/网关侧统计落盘增加占用重试与写盘串行化（旧快照不再复活已清零统计）；重置端点鉴权改为与其余管理端点一致的会话鉴权

## v1.5.0（2026-08-17）

### 🚀 新功能：auto 虚拟模型（智能选路，实例池设置弹层配置，默认关闭）

- 客户端只填 `model: "auto"`：网关按「用户权重 × 实测成功率」自动选模型，失败沿候选链无感降级（有界，最多切 3 个）
- 三种策略：均衡（平滑加权轮询，按权重分流额度）/ 速度优先（权重≥5 中选实测最快）/ 能力优先（按权重锁定，失败才降）
- 权重按模型粒度（0~10，0=永不参与，缺省 5，同模型跨实例同权重）；实例（节点）选择仍由 smart/failover 路由负责
- **上下文护栏三道防线**：请求侧保守 token 估算（中/英文自适应，图像不计入）；模型侧有效上限 = min(用户配置, 失败学习值, 保守默认 128k)；整条候选链按 est≤上限×0.9 过滤，全超限按最大上下文兜底
- 失败学习：上游报上下文类错误自动收紧该模型上限（填错真实值会被自动修正）
- 实测反馈滑窗（模型×实例，10 分钟）：auto 与非 auto 流量共同喂养，坏模型自动降权、恢复自动回升，冷启动不惩罚
- 每模型一行同处配置「权重 + 上下文上限」；保存即热生效（子进程 3s 热重载，无需重启）
- 调用日志新增 `auto_pick` 事件：记录选了谁、为什么、降级链是什么
- 三入口全支持：OpenAI Chat / Claude Messages / OpenAI Responses（含流式）

## v1.4.0（2026-08-16）

### 🚀 新功能：自定义模型源（新页「自定义模型」）

- 自带 API key 接入第三方模型供应商，四大协议全支持：OpenAI Chat Completions / Anthropic Messages / OpenAI Responses / Google Gemini
- 多 key 池：轮询 + 错误转移调度，429 自动冷却、401 自动禁用换 key
- 向实例/网关子进程热传播：/v1/models 不重启即生效
- 模型目录磁盘缓存：重启后首个请求即含自定义模型
- 编辑回填已存 key、白名单暴露、源活性探测
- 数据清理与自定义模型解耦，清空缓存不再误伤自定义源

### ⚡ 性能与路由

- 竞速整体预算 + 冷启动不竞速；自适应竞速（按压力动态调整副本数）
- 429 感知：降级单发 + 指数退避 + 可见报错
- 质量自愈分类型恢复：链路类自动自愈 / 账号类（401/402/429）永久禁用
- 上游代理出口配置：绕过本机出口 IP 风控
- 调用日志补齐 + 探活目标可配

### 🛡️ 稳定性（并发审计 27 项 + Code Review 33 项全数修复）

- 请求取消穿透下游、http.Client 复用、统计/日志写盘单写者
- 网关启动互斥、账号借出互斥、订阅每源独立调度
- 熔断半开计时、配置热重载原子化等边界修复

### 🎨 UI/UX

- 全局任务悬浮窗：右下角多任务栈，跨页常驻
- 实例池状态筛选、轮询间隔可配（0=关闭）、日志页分页、统计页迷你条形图
- 停止扫描交互重做（三态+进度）+ 扫描/停止并发配置面板
- 节点池轮询自杀 bug、订阅导入伪节点过滤（与 Clash 对齐）、删除订阅原子化清理

### 🔧 重要修复

- **安装器**：NSIS 保持 perMachine 全用户安装（安装路径与历史版本一致），并在安装后自动授予安装目录写权限——修复此前安装后「127.0.0.1 拒绝连接」的问题，无需先卸载旧版
- 面板请求空 body（清空/保存配置失效）、日志页空日志报错

### 📦 安装包

Windows（NSIS）/ Linux（deb、AppImage）/ macOS（dmg）

## v1.3.3（2026-08-13）

- 保留 Clash/订阅节点 ws Host 头，修复 CDN 节点 403（#8，致谢 383827453-max）
- README 更新（多端部署/订阅管理/性能模式/批量操作/构建流程）与效果图跟踪恢复

## v1.3.2（2026-08-13）

- T1 一键重启仅重启运行中的池成员；T2 实例池批量按钮纯勾选驱动；T3 订阅源列表管理（多条增删拉取+按分组释放进度）；T4 网关密钥设置后立即生效
- V1 订阅解析统一过滤公告/信息伪节点；V2 删除订阅前确认使用中实例数；V3 独享页批量释放；V4+V5 独享页链路质量列/探测开关；直连请求显示「直连」
- sing-box 下载增加重试与镜像兜底

## v1.3.1（2026-08-13）

- Go core 底座迁移收官：订阅/健康巡检/导出/日志过滤/节点删除/网关密钥/headless/前端全部迁移至 /api/admin HTTP
- 端口槽位表（40000/44100/48200/52300）+ 实例/sing-box 端口避让；统一网关端口 18080→40080
- Linux/macOS/Windows 三平台打包矩阵（deb/appimage/dmg/nsis）+ WebView2Loader.dll 纳入仓库
- 性能模式（链路探活/质量加权/熔断半开）、请求级竞速（P2b）、残留进程探测与一键清除
- UI：设置页卡片归位/折叠面板、路由模式分三行、退出二次确认、并发设置可配

## v1.3.0（2026-08-09）

### 🎉 特别致谢

- **感谢 [FYHC1](https://github.com/FYHC1)**：v1.3.0 的核心贡献者——完整实现了 **Linux / Headless 支持**（`serve` 子命令、systemd 服务、跨 WSL/Windows 构建）、**浏览器 WebUI**（桌面与 headless 共用 axum HTTP 层）、**前端嵌入 exe**（单文件自包含，无需 dist 伴行）、**订阅拉取与批量导入**、**健康巡检自动重启**、**报表导出**、**配置化**（网关端口/密钥、HTTP 端口）等一系列能力，是本版本 Linux/WebUI 架构的基石。
- **感谢 [Sujinxin123](https://github.com/Sujinxin123)**：v1.0.0「统一网关、免费额度实测健康检查、代理池健康检查、配置热更新、模型必填、同模型重试、批量并行启停」等核心能力均移植自该项目。
- **感谢 [6Kmfi6HP](https://github.com/6Kmfi6HP)**：Go 代理核心（OpenAI/Anthropic/Responses 协议转换）的开源贡献。
- 感谢所有贡献者与使用者的反馈与支持。

### 🚀 新功能（Linux / Headless）

- **Linux 桌面**：支持 .deb / AppImage 安装包
- **Headless 模式**：`opencode2api serve --port 19090` 纯浏览器管理，systemd 一键部署
- **WebUI**：桌面与 headless 共用本地 HTTP 服务（`127.0.0.1:19090`），浏览器可完整管理
- **前端嵌入 exe**：单文件自包含，无需磁盘 `dist/` 伴行
- **订阅拉取与批量导入**（Clash YAML / base64 / v2ray），支持后台自动拉取
- **健康巡检**：周期 TCP 探测 + 失败自动重启 + `health.json` 持久化

### 🚀 新功能（通用）

- **节点池重设计**：扫描结果弹窗一键「入实例池 / 设为独享」，自动去重
- **并行扫描提速**（默认 8 并发，可调）
- **日志分析**：时段分析、节点分析视图（纯 CSS 条形图）、按天筛选、一键清空
- **重置 Token 统计**（二次确认，可清除已删除节点历史）
- **报表导出**：日志 CSV / 实例 JSON / 统计 JSON
- **配置化**：`config.json` 支持网关端口/密钥（热生效）、HTTP 端口、订阅、巡检、日志过滤
- **自定义统一网关密钥**（校验 + 热生效 + 前端更换交互）

### 优化

- 架构重构：`AppCore` 纯逻辑核心，桌面 command 与 headless HTTP 完全同源
- 修复 release 版误加载 devUrl 导致 localhost 拒绝连接的问题
- 非 Windows 平台 autostart 修复

## v1.2.0（2026-08-08）

### 新增

- 节点池「扫描 → 入实例池/设为独享」弹窗一键操作，自动去重
- 并行扫描提速（默认 8 并发）
- 日志页分析视图：时段分析、节点分析（纯 CSS，轻量）
- 重置 Token 统计（二次确认，可清除已删除节点）
- 日志按天筛选、一键清空
- 独享/实例池测试结果行内徽章，批量测试并行执行

### 优化

- 节点池：组全选跳过已实例化、入池后自动清勾选、徽章区分实例池/独享、选中行高亮
- 凭据仅无凭据时显示；重置弹窗文案简化

### 修复

- 修复节点池入池后勾选态残留无法取消
- FAQ 补充 400 报错日志菜单显示；README 增补轻量化原则

## v1.1.0（2026-08-06）

### ✨ 智能路由（默认开启）

- **smart 默认路由模式**：在故障转移基础上叠加健康计数——节点故障自动计数，连续 3 次自动停用该节点并切换，正常轮次自动清零，全程无需手动干预
- **坏状态码自动切换**：遇到 401/402/429/503 自动切换下一个健康节点
- **流内超时自动续写**：首字等待超时（10s）或回复静默（5s）自动切换节点并断点续写，对话不中断；长任务不受全局超时限制

### 🆕 新增功能

- **全流程调用日志**：成功/失败/中断均完整记录，日志页可按天查看
- **清除数据**：设置页一键清理（运行数据 / 实例记录 / 全部重置，全量重置自动备份配置），解决环境残留异常
- **一键重启实例池**：全停 → 强制释放被占端口 → 全部重启 → 网关同步，端口占用一键解决
- **配置即时生效**：修改路由/超时等配置自动生效，无需手动重启

### 🐛 修复与优化

- **SSE 流式兼容性**：事件按标准空行分隔，修复 IDE 等严格客户端解析失败
- **token 统计修正**：流式响应只记录最终用量，不再逐 chunk 累加
- **退出彻底清理**：退出时自动停止统一网关与全部实例（含残留进程），端口不再后台常驻
- **日志页修复**：修复日志页无数据问题

## v1.0.2（2026-08-05）

### 🐛 Bug 修复

- **sing-box 生成配置修复**：生成配置不再包含非法 `transport {type:tcp}`，修复扫描时 sing-box 启动即退出的问题（FIX_LOG 3.1）（贡献：FYHC1）
- **统一网关 token 统计修复**：网关进程未设置工作目录导致 token 统计错位落盘应用 exe 目录，统计界面读不到网关流量；现已对齐到 `runtime/_unified-gateway/`（贡献：FYHC1）
- **统一网关按节点拆分 token 统计**：界面新增「调用节点明细」，按 SOCKS5 出口节点聚合显示各节点请求数与 token 用量；直连实例不再产生无效的 `node_stats.json`（贡献：FYHC1）

### 🙏 致谢

感谢 [FYHC1](https://github.com/FYHC1) 对本版本的两项 Bug 修复贡献（#2 sing-box 配置、#3 统一网关 token 统计）。



## v1.0.1（2026-08-04）

### 🐛 Bug 修复

- **实例池全部启动/停止卡顿**：改为复用 Rust 并行命令（batch_start 4 worker / batch_stop 8 worker），不再前端逐个串行等待
- **节点池重复添加**：批量添加按 node 去重，同一节点只允许例化一次（后端拒绝 + 前端过滤已添加节点）
- **实例池统计口径**：「池内 x / 共 y」中的「共 y」只统计池成员，不再混入独享实例



## v1.0.0（2026-08-04）

### 亮点

首个正式版：在 A 多实例管理的基础上，融合 B 的 7 项核心能力，形成「独享实例 + 统一实例池」双容器管理模型。

### 🆕 新增功能

- **统一网关**：实例池聚合入口 `127.0.0.1:40080`，failover / round_robin 两种路由模式，一键启停
- **免费额度实测健康检查**：启动即探测免费模型可用性（名称含 `-free` 或为 `big-pickle`）
- **配置热更新**：修改运行配置不重启进程、不打断进行中的 SSE 流
- **模型必填校验**：chat / claude / responses 缺 model 一律返回 400
- **同模型失败重试**：同一模型路由失败自动重试；代理池健康检查 + 坏代理自动冷却
- **批量并行启停**：多 worker 并行，批量操作大幅提速；节点扫描并行探测

### 🎨 界面与体验

- 三页边界清晰：**独享**（仅独享实例启停/测试/释放）/ **实例池**（池成员启停、一键测试、释放、网关管理）/ **节点池**（扫描 + 独享或进池批量添加）
- 搜索、状态筛选、一键测试、密钥一键复制（API 地址 + 密钥合并复制）
- 移除即释放：自动关闭实例并释放节点，无中间态

### 🛠 其他

- GitHub Actions 自动构建 Windows NSIS 安装包并发布 Release
- 完整合并交付文档（见 docs/ 与 .tokeny/plans/）

### 🙏 致谢

感谢 [Sujinxin123](https://github.com/Sujinxin123)——本版本「统一网关、免费额度实测健康检查、代理池健康检查、配置热更新、模型必填、同模型重试、批量并行启停」等核心能力均移植自该项目，是本次合并的基石。
