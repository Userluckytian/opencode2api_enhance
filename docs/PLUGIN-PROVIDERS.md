...
- **非状态行必须被忽略，不得触发重启**（2026-09-21 修复）：就绪之后子进程若又打了一行非
  `ready`/`need_config`/`fatal` 的 JSON（插件的扩展状态行如 `panel_ready`/`panel_skip`、
  或重复的 `ready`），宿主应留在稳定态继续消费；把这类行当成「该重启了」会把健康的子进程
  反复杀掉重拉（loomy 插件曾因此每 1~2 秒重启一次、`restart_count` 无上限增长）。
  契约仍要求 stdout 只打状态行（见 §5），但宿主对此必须有容错。
